...
public class StatementController {
    ...
    public ResponseEntity<?> getFarmerStatements(@PathVariable String farmerNumber) {
        try {
            ...
            e.printStackTrace();
            return ResponseEntity.internalServerError().body("Error fetching statements.");
        }
    }
    ...
    public ResponseEntity<?> getManagerStatements(@PathVariable String managerUsername) {
        try {
            ...
            List<MonthlyStatement> managerStatements = allStatements.stream()
                ...
                // 🚨 THE CRITICAL FIX: We sort by ID descending. It achieves the exact same chronological sorting, but is 100% immune to NullPointerExceptions!
